...
class AgenteOpenAIFunctions:
    ...
    def criar_qa_chain(documentos):
        ...
        split_textos = quebrador.split_documents(documentos)

        vectordb = FAISS.from_documents(
            documents=split_textos,
            embedding=embedding_model
        )


        qa_chain = RetrievalQA.from_chain_type(llm, retriever=vectordb.as_retriever(search_kwargs={"k": 5}))
